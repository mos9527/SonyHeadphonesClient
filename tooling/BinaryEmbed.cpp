// From https://github.com/ocornut/imgui/blob/master/misc/fonts/binary_to_compressed_c.cpp, with modifications
// Helper tool to turn a file into a C array, if you want to embed font data in your source code.
// For use with ImGui's AddFontFromMemoryCompressedTTF
...
static bool binary_to_compressed_c(const char* filename, const char* symbol, FILE* out, int& array_size);

int main(int argc, char** argv)
{
    if (argc != 5)
    {
        printf("Syntax: %s <inputfile> <symbol> <out .h file> <out .c file>\n", argv[0]);
        return 1;
    }
    FILE* fout = fopen(argv[4], "wb");
    int array_size;
    binary_to_compressed_c(argv[1], argv[2], fout, array_size);
    fclose(fout);
...
bool binary_to_compressed_c(const char* filename, const char* symbol, FILE* out, int& array_size)
{
    // Read file

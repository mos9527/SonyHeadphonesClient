...
constexpr bool IsMdrHResult(HRESULT hr)
{
    return HRESULT_FACILITY(hr) == FACILITY_MDR;
}
...
{
    int mask = 0;
    if ((props & GattCharacteristicProperties_Read) != GattCharacteristicProperties_None) mask |= GATT_PROP_READ;
    if ((props & GattCharacteristicProperties_Write) != GattCharacteristicProperties_None) mask |= GATT_PROP_WRITE;
    if ((props & GattCharacteristicProperties_WriteWithoutResponse) != GattCharacteristicProperties_None) mask |= GATT_PROP_WRITE_NO_RESP;
    if ((props & GattCharacteristicProperties_Notify) != GattCharacteristicProperties_None) mask |= GATT_PROP_NOTIFY;
    if ((props & GattCharacteristicProperties_Indicate) != GattCharacteristicProperties_None) mask |= GATT_PROP_INDICATE;
    return mask;
}
...
struct MDRConnectionWindowsBLE
{
    ...
    static int Connect(void* user, const char* macAddress, const char* serviceUUID) noexcept
    {
        ...
        ptr->connectThread = std::jthread([ptr, btAddr, svcUUID](std::stop_token stop)
        {
            ...
            auto inner = [&]() -> HRESULT
            {
                ...
                if (stop.stop_requested()) return E_ABORT;
                ...
                RETURN_IF_FAILED(spBleDevice3->GetGattServicesWithCacheModeAsync(BluetoothCacheMode_Uncached, &spServicesOp));
                ...
                RETURN_IF_FAILED(spService3->GetCharacteristicsWithCacheModeAsync(BluetoothCacheMode_Uncached, &spCharsOp));
                ...
                for (uint32_t i = 0; i < cCharacteristics; i++)
                {
                    ...
                    MDR_LOG("[BLE]   Characteristic: {} [{}]", GuidToString(uuid), PropertiesToString(PropertiesToBitmask(props)));
                    ...
                    if (ptr->writeChar == nullptr &&
                        ((props & GattCharacteristicProperties_Write) != GattCharacteristicProperties_None ||
                         (props & GattCharacteristicProperties_WriteWithoutResponse) != GattCharacteristicProperties_None))
                    {
                        ptr->writeChar = spCh;
    ...
    static void Disconnect(void* user) noexcept
    {
        ...
        HRESULT hr = RunOnMTA([ptr]() -> HRESULT
        {
